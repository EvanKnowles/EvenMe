 ...
 *
 * Usage (concentric rings — default):
 *   java PhotoConverterMain <input> <output> [interval] [lineThickness] [hueShift] [blurSigma] [perturbStrength] [centerX] [centerY]
 *
 * Usage (luminance contours):
 *   java PhotoConverterMain <input> <output> --contour [interval] [lineThickness] [hueShift] [blurSigma]
 *
 * Concentric ring defaults:
 *   interval        = 8      pixels between rings
 *   lineThickness   = 1      separator width in pixels
 *   hueShift        = -0.12  negative → blue/purple
 *   blurSigma       = 2.0    Gaussian smoothing on luminance
 *   perturbStrength = 30     how strongly rings warp around image features
...
public class PhotoConverterMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length > 2 && args[2].equals("--contour")) {
            ...
            float perturb   = args.length > 6 ? Float.parseFloat(args[6])   : 30f;
    ...
    private static void printUsage() {
        System.err.println("Usage: PhotoConverterMain <input> <output> [interval] [lineThickness] [hueShift] [blurSigma] [perturbStrength] [centerX] [centerY]");
